...
'''
Function: Super directive beamforming. This version is adapted from the
...
Example folder:
	...
	/share/spandh.ami1/usr/yulan/dev/btk/tools/SDBF/dbg/

Example command:

	./SDBeamforming  --cfg=dbg.cfg

Yulan Liu, 21 Mar 2016
'''
...
class SDBeamformer():
    '''
    Super directive beamforming in sub-band domain.
    '''
    ...
    def run_beamforming(self, inwav, posFile=None, TDOAFile=None, mode=None, outaudio_prefix=None, overlapSample=1024, myu=0.01):
        '''
...
#            tmp_pos = numpy.array(Pos[i])       # [DEBUG]
	    ...
	    # Set the diffuse noise covariance matrix
            self.pBeamformer.setDiffuseNoiseModel( self.micPositions, self.sampleRate, self.sspeed )
            ...
            for chX in range(self.chanN):
                if self.chanNwav==1:
                    ...
                    if not os.path.exists(sampleFile):
                        print '[WARNING] Can not find file: %s .' % sampleFile
...
def main():
    ...
    beamformer = SDBeamformer(options)
